...
> = {
  ...
  detail: { position: [-4.5, 1.8, 6.4], target: [-0.3, 1.2, -0.7], fov: 30 },
  workstation: {
    position: [-3.8, 1.85, -3.4],
    target: [-1.1, 0.87, -1.1],
    fov: 46,
  },
  wall: { position: [-0.35, 1.85, -0.9], target: [-0.35, 1.95, -4], fov: 46 },
};
...
function displayTexture(workstation: boolean) {
  ...
  const items = [
    ["住院患者", "32 人", "今日入院  3", "今日出院  2"],
    ["护理工作", "12 项", "待执行  2", "已完成  10"],
    ["重点关注", "5 人", "一级护理  2", "特殊观察  3"],
  ];
...
export function createStationPreview(host: HTMLElement) {
  ...
  const camera = new THREE.PerspectiveCamera(33, 1, 0.03, 100);
  ...
  environment.dispose();
  pmrem.dispose();
  scene.add(new THREE.HemisphereLight(0xe6f1ff, 0xb3a28a, 0.6));
  RectAreaLightUniformsLib.init();
  for (const [x, y, z, intensity, width, height] of [
    [-3.7, 2.9, 4, 2, 4.8, 2],
    [5.8, 2.6, 0, 2, 2, 2.5],
    [0, 2.85, -2.9, 2, 6.5, 0.8],
  ]) {
  ...
  const key = new THREE.SpotLight(0xfff7eb, 45, 25, Math.PI / 2.7, 0.85, 2);
  ...
  function resize() {
    ...
    camera.fov = THREE.MathUtils.radToDeg(
      2 *
        Math.atan(Math.tan(baseFov / 2) * Math.max(1, 16 / 9 / camera.aspect)),
    );
  ...
  return {
    async load(onProgress: (percent: number) => void) {
      ...
      textures.add(workTexture);
      screens.forEach((object, index) => {
        const material = new THREE.MeshBasicMaterial({
          map: index === 0 ? mainTexture : workTexture,
          toneMapped: false,
        });
        materials.add(material);
      ...
      if (clock) {
        ...
        materials.add(material);
        const geometry = new THREE.CircleGeometry(0.222, 64);
        ...
        face.position.z = bounds.max.z + 0.001;
        scene.add(face);
      }
    ...
    setExposure(value: number) {
      ...
      renderer.toneMappingExposure = THREE.MathUtils.clamp(value, 0.6, 1.6);
      invalidate();
    },
