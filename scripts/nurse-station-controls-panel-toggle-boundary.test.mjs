...
assert.match(app, /<DashboardLeftPanel v-if="panelsVisible && isNurseStation"/);
assert.match(app, /:panels-visible="panelsVisible"/);
assert.match(app, /v-if="isWardInterior && currentWard && panelsVisible"/);
assert.match(app, /v-if="activeAlertTask && isWardInterior && panelsVisible"/);
assert.match(app, /<WardLegend v-if="isWard && panelsVisible"/);
