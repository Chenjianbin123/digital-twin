...
assert.match(areaScene, /new THREE\.RectAreaLight\(0x9eefff, 0\.85, 5\.8, 1\.2\)/);
assert.match(areaScene, /const primaryScreenKinds = new Set<NurseStationBoardKind>/);
assert.match(areaScene, /const overlayOpacity = primaryScreenKinds\.has\(kind\) \? 1 : 0\.78;/);
assert.match(areaScene, /material\.envMapIntensity = 0\.56/);
