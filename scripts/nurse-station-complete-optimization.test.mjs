...
assert.match(panel, /:max-items="8"/);
assert.doesNotMatch(panel, /运行指数/);
assert.match(app, /@model-state="scenes\['nurse-station'\]\.onState"/);
...
assert.doesNotMatch(app, /transform: translateX\(-50%\) scale\(0\.9\);/);
...
assert.match(areaScene, /this\.configureCorridorShadowCasters\(model\);\s*this\.fitCorridorKeyShadow\(model\);/);
assert.match(areaScene, /new THREE\.DirectionalLight\(0xffefd4, 0\.46\)/);
assert.match(areaScene, /new THREE\.RectAreaLight\(0xffefd4, 0\.42, 5\.8, 1\.2\)/);
assert.match(areaScene, /new THREE\.RectAreaLight\(0xfff3d4, 5\.4,/);
assert.match(areaScene, /lit\.color\.set\(dark \? '#3f7384' : '#2b6c82'\)/);
assert.match(areaScene, /const overlayOpacity = 1;/);
assert.match(areaScene, /const envMapIntensity = options\?\.envMapIntensity \?\? 0\.56;/);
assert.match(areaScene, /preserveEnvMapIntensity: IS_REFERENCE_STATION/);
...
assert.match(header, /v-if="operatorName"/);
assert.match(header, /operatorRole \? ` · \$\{operatorRole\}`/);
